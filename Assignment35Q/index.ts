...
let AppleTotal:number=10;
let AppleGive:number=7;
let AppleLeft:number=AppleTotal-AppleGive;
...
function ArraywithA(StrList:string[]):string[]{
...
let scores=[10, 5, 20, 20, 4, 5, 2, 25, 1];
  function MinMaxScores(SC:number[])
  {
    let Exceed = SC[0];
    let Bellow = SC[0];
    let ExceedTime = 0;
...
function sumOfElements(arr:any, isEven:boolean) {
    let sum = 0;
...
function ElementExistence(arr:any, target:any) {
...
function findMin(arr: any[],n: number)
{
    let mini = arr[0];
    for (let i = 0; i < n; i++) {
        if (arr[i] < mini) {
...
function filterByLength(arr: any[], n: number) {
...
function countOccurrences(arr:number [], element:number) {
...
function fahrenheitToCelsius(temperatures: string | any[]) {
...
function logIncompleteTasks(list:any[]) {
...
const strings: string[] = ["apple", "mango", "water melon"];
...
function calculate(num1:number, operator:string, num2:number) {
